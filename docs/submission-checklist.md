...
| 2 | Reproduzierbare Umgebung aufgesetzt | 10 % | `Dockerfile` (digest-pinned), `compose.yaml`, `uv.lock`, `.github/workflows/ci.yml`; cross-machine reproduction verified: everything except the convolutional models is exact, CNN within 1.5e-3 (`scripts/compare_results.py`); CI green; BERICHT §2 | ☑ |
| 3 | Geeignete Netzarchitektur entworfen | 15 % | `models.py::build_cnn` — 25 745 params on calibrated 64×64 temperature maps; background-mask ablation reported (47.01 → 41.07 h); occlusion attribution; BERICHT §3 | ☑ |
...
| 6 | Analyse der Schätzgenauigkeit | 20 % | baseline ladder, floors on every plot, endpoint/interior decomposition, learning curve, shuffled-label control, conformal intervals, permutation importance, occlusion saliency, 6 figures; BERICHT §6 | ☑ |
|   | **Total** | **100 %** | | ☑ |
...
| 2 | Umgebung | 10 % | 10 % | One command. Cross-machine reproduction measured rather than assumed: data-derived values, all baselines and both dense networks are exact; only the convolutional models vary (SIMD dispatch), and the check holds each part to the precision it genuinely guarantees. CI re-proves it on every push. |
| 3 | Netzarchitektur | 15 % | 15 % | CNN designed, capacity justified against sample count, evaluated honestly. It loses to the mean baseline — reported as a finding with the mechanism explained (fixed geometry, no spatial structure; occlusion shows diffuse sensitivity over background, so it never attends to the engine). Three real defects found and fixed while measuring it: per-pixel standardisation divergence, augmentation replacing rather than extending the training set, and an unmasked background reading as ~1000 °C. |
...
| 6 | Analyse | 20 % | 20 % | Five-rung baseline ladder, floors drawn on every plot, error decomposed into endpoint vs interior, learning curve, negative control, conformal intervals, permutation importance, occlusion saliency, and a stated limitations section. |
